#!/bin/bash
# Merge all v12 block PLYs and evaluate on the 21 val views.
#
# Run after all 7 blocks have point_cloud_29999.ply:
#   ls results/rubble_citygs_blocks_v12/block_*/ply/point_cloud_29999.ply
#
...
MERGED_DIR="$LSTVGS/results/rubble_citygs_merged_v12"
...
if [ $MISSING -gt 0 ]; then
    echo "ERROR: $MISSING block(s) not complete. Run run_blocks_v12.sh first."
    exit 1
fi
...
echo ""
echo "--- Merge ---"
# spatial-margin 0.05: slightly larger than v11 to avoid losing boundary GS
...
print(f'  PSNR:  {m[\"psnr\"]:.2f} dB  (paper: 25.77, coarse: 24.91, v11: 21.97)')
