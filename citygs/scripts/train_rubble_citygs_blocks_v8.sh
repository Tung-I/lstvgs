#!/bin/bash
#SBATCH --job-name=rubble-citygs-blocks-v8
#SBATCH --partition=gpu
#SBATCH --gres=gpu:l40s:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=/work/pi_rsitaram_umass_edu/tungi/lstvgs/results/rubble_citygs_blocks_v8/slurm_%j.log
#SBATCH --error=/work/pi_rsitaram_umass_edu/tungi/lstvgs/results/rubble_citygs_blocks_v8/slurm_%j.log
...
BLOCK_DIR="$WORK_DIR/lstvgs/results/rubble_citygs_blocks_v8"
MERGE_DIR="$WORK_DIR/lstvgs/results/rubble_citygs_merged_v8"
...
echo "=== Rubble CityGS block training v8 (NO opacity reset, expanded-box) started: $(date) ==="
...
echo "Using coarse PLY: $COARSE_PLY (v3, 981k Gaussians, 14.63 dB)"
...
# Re-use v3's partition (already computed with --simple-selection 1.5)
if [ -f "$PARTITION_DIR/partition_info.json" ]; then
    echo "Using existing v3 partition from: $PARTITION_DIR"
else
