#!/bin/bash
#SBATCH --job-name=rubble-citygs-blocks-v7
#SBATCH --partition=gpu
#SBATCH --gres=gpu:l40s:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=/work/pi_rsitaram_umass_edu/tungi/lstvgs/results/rubble_citygs_blocks_v7/slurm_%j.log
#SBATCH --error=/work/pi_rsitaram_umass_edu/tungi/lstvgs/results/rubble_citygs_blocks_v7/slurm_%j.log
...
# Use v3 coarse PLY (981k Gaussians, 14.63 dB) — best coarse model
COARSE_DIR="$WORK_DIR/lstvgs/results/rubble_citygs_coarse_v3"
BLOCK_DIR="$WORK_DIR/lstvgs/results/rubble_citygs_blocks_v7"
MERGE_DIR="$WORK_DIR/lstvgs/results/rubble_citygs_merged_v7"
...
echo "=== Rubble CityGS block training v7 (SSIM partition + v3 coarse) started: $(date) ==="
...
echo "Using coarse PLY: $COARSE_PLY (981k Gaussians, 14.63 dB)"

PARTITION_DIR="$COARSE_DIR/partition_ssim"
echo ""
echo "=== Step 1: SSIM-based partitioning ($(date)) ==="
echo "Using SSIM-based camera assignment (no --simple-selection) for balanced distribution"
cd "$WORK_DIR/lstvgs"
