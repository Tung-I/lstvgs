#!/bin/bash
#SBATCH --job-name=rubble-coarse-sfm
...
echo "=== Rubble CityGS coarse (SFM init) started: $(date) ==="
...
if [ "$N_POINTS" -lt 1000 ]; then
    echo "ERROR: Insufficient SFM points ($N_POINTS). Run colmap_triangulate_rubble.sh first."
    exit 1
fi
