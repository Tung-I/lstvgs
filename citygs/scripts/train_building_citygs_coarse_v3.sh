#!/bin/bash
#SBATCH --job-name=building-citygs-coarse-v3
...
echo "=== Building CityGS coarse v3 training started: $(date) ==="
echo "Host: $(hostname), GPU: $(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader)"

cd "$EXAMPLES"

# v3: Reduced init_extent=1.0 (was 3.0) and increased init_num_pts=500k (was 200k).
# Building uses halved position LR (CityGaussian config: position_lr_init=0.00008).
# means_lr=0.011 * scene_scale(2.121) ≈ 0.023 effective LR, matching CityGaussian building.
...
echo "=== Building Coarse v3 training finished: $(date) ==="
