#!/bin/bash
#SBATCH --job-name=rubble-citygs-blocks-v6
#SBATCH --partition=gpu
#SBATCH --gres=gpu:l40s:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --output=/work/pi_rsitaram_umass_edu/tungi/lctvgs/results/rubble_citygs_blocks_v6/slurm_%j.log
#SBATCH --error=/work/pi_rsitaram_umass_edu/tungi/lctvgs/results/rubble_citygs_blocks_v6/slurm_%j.log
...
# Use v5 coarse PLY (continued training from v3) for better init quality
COARSE_DIR="$WORK_DIR/lctvgs/results/rubble_citygs_coarse_v5"
BLOCK_DIR="$WORK_DIR/lctvgs/results/rubble_citygs_blocks_v6"
MERGE_DIR="$WORK_DIR/lctvgs/results/rubble_citygs_merged_v6"
...
echo "=== Rubble CityGS block training v6 (SSIM partition) started: $(date) ==="
...
PARTITION_DIR="$COARSE_DIR/partition_ssim"
echo ""
echo "=== Step 1: SSIM-based partitioning ($(date)) ==="
echo "Note: This uses SSIM to assign cameras to blocks (more balanced than expanded-box)"
cd "$WORK_DIR/lctvgs"
...
# Note: no --simple-selection, so SSIM-based assignment is used
