#!/bin/bash
# Resume block finetuning — skips blocks that already have point_cloud_29999.ply
# Usage: bash run_blocks_resume.sh
...
LOG="$BLOCK_DIR/blocks_resume.log"
...
cd "$EXAMPLES"
echo "=== Block finetuning resume started: $(date) ===" | tee -a "$LOG"
...
echo "" | tee -a "$LOG"
echo "=== Resume run complete: $(date) ===" | tee -a "$LOG"
